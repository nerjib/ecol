		   ...
		   <div class="timeline-heading">
			...
			// check the subject been thought in that class
				$s1trsql="SELECT * FROM $schuname WHERE accttype='subject' && class='$mclass'"or die("mysqli_error()");
					...
					<td width="500px" align="center"><?php echo $score2; } ?></td> <td width="500px" align="center"><?php
					echo $score3 ;   } ?></td></tr><?php //}  ?></table>
</form>
...
<?php
$s1trsql="SELECT * FROM assessment WHERE schuname='$schuname' && class='$mclass' && term='$term' && year='$session' "or die("mysqli_error()");
$r1s=mysqli_query($db,$s1trsql);
$tot=0;
while($r1ow=mysqli_fetch_array($r1s)){
	$score1=$r1ow['score'];
$tot=$tot + $score1;
		...
		//getting cumulative tootal
if($counter==0){
	$average="";
}
else{
$average=round($tot/$counter,2);
}
