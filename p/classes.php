...
while($row=mysqli_fetch_array($rs)){
	...
	//	$lname=$row['lname'];
						...
						<a href="classsub.php?class1=<?php echo $name ?>"><?php echo $name ?></a>
...
<?php } ?>
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header" style="text-transform:uppercase;"><?php echo "$mclass assessment" ?></h1>
                    </div>
                </div>
                <!-- /.row -->
									   
  <table border="bold" width="100%" style="text-transform:uppercase; " >
		 <tr style="text-transform:uppercase; font-size:8px" ><td width="10px" align="center">S/No</td><td width="100px" align="center">Admission No</td>
		 <td align="center" width="500px">Name</td>
									   <?php
									   $db=mysqli_connect('localhost','root','','ecole') or die(mysqli_error());
 $strsql="SELECT * FROM $uname WHERE accttype='subject' && class='$mclass'"or die("mysqli_error()");
$rs=mysqli_query($db,$strsql);
$counter=0;
while($row=mysqli_fetch_array($rs)){
	$name=$row['name'];
	
?><td width="50px" align="center" ><?php echo $name ; }?></td><td align="center">Total</td></tr>
				<?php
$db=mysqli_connect('localhost','root','','ecole') or die(mysqli_error());
...
?>
		   ...
		   <div class="timeline-heading">
			...
			// check the subject been thought in that class
				$s1trsql="SELECT * FROM $uname WHERE accttype='subject' && class='$mclass'"or die("mysqli_error()");
					...
					while($r1ow=mysqli_fetch_array($r1s)){
						...
						$name=$r1ow['name'];
							...
							//		$title=$row['title'];			
														...
														$s2trsql="SELECT * FROM assessment WHERE schuname='$uname' && class='$mclass' && term='$term' && subject='$name' && year='$session' && admissionno='$admno'"or die("mysqli_error()");
														...
														$s3trsql="SELECT * FROM assessment WHERE schuname='$uname' && class='$mclass' && term='$term' && year='$session' && admissionno='$admno'"or die("mysqli_error()");
